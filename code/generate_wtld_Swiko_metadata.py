...
import nltk
from cassis import *
from dkd_meta import dakoda_metadata_scheme_2025_05_26 as meta
...
GENDER_MAP = {"f": "female", "m": "male", 0: "notAvailable", "NA": "notAvailable"}
...
def get_lan_info_for_iso_code( lan_string: str, lang_info_df: pd.DataFrame) -> Tuple[str, str]:
	...
	lang_df = lang_info_df[lang_info_df["Orig_code_ISO"] == lan_string]
	if len(lang_df)==1:
		lang_row = lang_df.iloc[0]
		lan_name_en = lang_row["iso639_3_en"]
		lan_name_de = lang_row["iso639_3_de"]
	else:
...
def lookup_word_order(df: pd.DataFrame, code: str) -> str:
	print(df.head())
	match_df = df[df["iso639_3_code"] == code]
	if len(match_df) == 0:
		return "notAvailable"
	else:
		assert len(match_df) == 1
		return match_df.iloc[0]["dominantWordOrder"]
...
def main():
	...
	# NB: we're filtering here for RI-Ids, these mean French as lang of schooling and texts in German
	...
	corp_meta_df = pd.read_excel(
		config["corpus"]["corpus_metadata_table"], engine="openpyxl"
	)
	...
	transcriber_df = pd.read_csv(config["corpus"]["transcriber_table"], sep="\t", quoting=3,names=["file","trans_sigle"])
	doc2trans_map =  {}
	for ix, row in transcriber_df.iterrows():
		...
		if shortname not in doc2trans_map:
			...
			logger.error("there are paper and computer versions of the same single task instance!")

		doc2trans_map[shortname]=row["trans_sigle"]

	logger.info(f"doc2trans_map: {len(doc2trans_map)} entries\n {doc2trans_map}")
	...
	# CSECTION: collect corpus level info
	...
	curr_corp_learner_proficiency_assignment_instrument_list = convert_cell_string_to_list(
	...
	curr_corp_production_setting_naturalistic_specific_list = convert_cell_string_to_list(
		curr_corp_production_setting_naturalistic_specific_string
	)

	# corpus_production_setting_researchProject: boolean
	curr_corp_production_setting_researchProject_val = map_to_boolean(corp_meta_df[
		corp_meta_df["corpus_acronym"] == corpus_name
	].iloc[0]["corpus_production_setting_researchProject"])
	...
	curr_corp_production_setting_participants_L1_L2_interaction_list = convert_cell_string_to_list(
	...
	# TODO: Tabelle sagt "dkd-Variable" als Hilfsdokument. mir nicht klar, was hier passieren soll.
	default_learner_proficiency_estimate = meta.NaString.NOT_AVAILABLE
	default_learner_majorSubject = meta.NaString.NOT_AVAILABLE
	default_learner_professionGroup = meta.NaString.NOT_AVAILABLE
	...
	for ix, docid in enumerate(sorted(docid_lines)):
		...
		mdf = learner_df[learner_df["PID"] == src_learner_id]
		if len(mdf) == 1:
			pass
		elif len(mdf)==0:
			logger.error(
				f"we don't get any match for the learner id {src_learner_id}: {mdf}"
			)
			continue
		else:
				logger.error( f"we don't get a unique match for the learner id {src_learner_id}: {mdf}")
				continue
		...
		ca.corpus_admin_contributor_dkd = [
			"Jamila Bläsing",
			"Luise Böttcher",
			"Shanny Druker",
			"Lisa Lenort",
		...
		# Q: really no info for SWIKO/WTLD?
		...
		cd.corpus_design_time_of_data_collection = mdf["yearcollected"].iloc[0]
		...
		cp.corpus_proficiency_learner_assignment_instrument = curr_corp_learner_proficiency_assignment_instrument_string      
		cp.corpus_proficiency_level = curr_corp_proficiency_level_val
		# TODO: check this
		cp.corpus_proficiency_text_assignment_instrument = curr_corp_text_proficiency_assignment_instrument_list

		cp.corpus_proficiency_text_automatic_assignment_instrument = curr_corp_proficiency_textAutomatic_instrument_val
		...
		cj.corpus_project_name_orig = "Schweizer Lernerkorpus SWIKO / WETLAND 🔥"
		...
		csub.corpus_subcorpus_size_learners = -1
		csub.corpus_subcorpus_size_texts = -1
		csub.corpus_subcorpus_size_tokens = -1
		...
		prod_set.production_setting_school_grade = mdf["schoolyear"].iloc[0]

		prod_set.production_setting_educational_stage = curr_corp_production_setting_educationalStage_list
		prod_set.production_setting_language_test = curr_corp_production_setting_language_testing_val

		prod_set.production_setting_language_course_level = curr_corp_production_setting_languageCourseLevel_list
		prod_set.production_setting_naturalistic = curr_corp_production_setting_naturalistic_specific_list
		prod_set.production_setting_in_research_project_collected = curr_corp_production_setting_researchProject_val
		...
		ti_block.task_interaction_conceptual_mode = curr_corp_production_setting_conceptualmode_list

		ti_block.task_interaction_expected_rhetorical_functions = [meta.NaString.NOT_AVAILABLE]
		...
		ti_block.task_interaction_participants = curr_corp_production_setting_participants_list
		...
		if  count_l1s(mdf.iloc[0], ["l1a", "l1b", "l1c"])>1:
		...
		learner_socio.learner_socio_gender = GENDER_MAP.get(mdf.iloc[0]["gender"])
		...
		learner_socio.learner_socio_school_grade = mdf.iloc[0]["schoolyear"]
		...
		canton = mdf.iloc[0]["canton"].lower()

		# NB: variable that records the (sub)corpus target language: curr_corp_subcorpus_target_language_val
		# CSUBSUBSECTION: L1(s)
		lgs_map   = {"deu":"Target language, L2"}
		l1s = []
		for lan in ["l1a", "l1b", "l1c"]:
			lan_string = mdf.iloc[0][lan]
			if (
		...
		for lan_string in lgs_map.keys():
				...
				lg_name_en, lg_name_de = get_lan_info_for_iso_code(
				...
				# TODO: lang status : L1, L2, or target language
				learner_language.learner_language_status = language_status_list
				learner_language.learner_language_dominant_word_order = (
					lookup_word_order(lang_info_df, lan_string)
				)
				learner_language.learner_language_is_spoken_home = (
					meta.NaString.NOT_AVAILABLE
				)
				learner_language.learner_language_is_spoken_school = (
					meta.NaString.NOT_AVAILABLE
				)
				learner_language.learner_language_parent_l1 = (
					meta.NaString.NOT_AVAILABLE
				)
				...
				if "Target language" in language_status_list:
					...
					try:
						# 
						curr_age = float(mdf.iloc[0]["text_learner_ageProduction"])
						curr_doi = float(mdf.iloc[0]["durationOfInstruction"])
						if school_lg == "f":
							lle.learner_language_exposure_onset= curr_age - curr_doi
						elif school_lg == "d":
							lle.learner_language_exposure_onset = meta.NaString.NOT_APPLICABLE
						else:
							lle.learner_language_exposure_onset = meta.NaString.NOT_AVAILABLE
					except:
						logger.error("either age or durationOfInstruction is not a number, skipping this entry")
						lle.learner_language_exposure_onset = meta.NaString.NOT_AVAILABLE
						curr_age = meta.NaString.NOT_AVAILABLE
						curr_doi = meta.NaString.NOT_AVAILABLE
						

					# TODO: Wertebereich festlegen für onset_group
					...
					if src_learner_id.lower().startswith("ri") and canton.lower() == "ri":
						lle.learner_language_exposure_duration_of_instruction = 6
					elif src_learner_id.lower().startswith("mo") and canton.lower() == "mo":
						lle.learner_language_exposure_duration_of_instruction = 6
					elif src_learner_id.lower().startswith("es") and canton.lower() == "es":
						lle.learner_language_exposure_duration_of_instruction = 5
					else:
						lle.learner_language_exposure_duration_of_instruction = meta.NaString.NOT_APPLICABLE
					...
					if school_lg == "f":
						lle.learner_language_exposure_input = "mainly in controlled teaching contexts"
					else:
						lle.learner_language_exposure_input = "mainly without controlled teaching processes"
					...
					if school_lg == "f":
						...
						lle.learner_language_exposure_institution == meta.NaString.NOT_APPLICABLE
					elif "deu" in l1s and "deu" in l1s:
						lle.learner_language_exposure_institution = meta.NaString.NOT_APPLICABLE
					else:
						lle.learner_language_exposure_institution = meta.NaString.NOT_AVAILABLE

					lle.learner_language_exposure_months_spent_environment = meta.NaString.NOT_AVAILABLE

					# {EINFÜGEN:[learner_target_language_onset]-EINFÜGEN:[text_learner_ageProduction]: instructed}
					lle.learner_language_exposure_learning_context = f"{lle.learner_language_exposure_onset}-{curr_age}: instructed"

					lle.learner_language_exposure_place_acquisition = "CHE"

					lle.learner_language_exposure_was_in_environment = meta.NaString.NOT_AVAILABLE
					...
					# TODO: the excel table has no mapping for this next variable
					llp.learner_language_proficiency_cTestCefr = meta.NaString.NOT_AVAILABLE

					llp.learner_language_proficiency_cTestPercent = default_learner_proficiency_c_test_percent
					llp.learner_language_proficiency_c_test_level_detail = default_learner_proficiency_c_test_LevelDetail
					llp.learner_language_proficiency_cTestType = default_learner_proficiency_c_test_type

					# we  have only one estimate so we treat it both as min and max
					llp.learner_language_proficiency_estimateMin = default_learner_proficiency_estimate
					llp.learner_language_proficiency_estimateMax = default_learner_proficiency_estimate

					llp.learner_language_proficiency_selfAssessment = default_learner_target_language_selfAssessment


					llp.learner_language_proficiency_assignment_instrument = default_learner_proficiency_instrument
					llp.learner_language_proficiency_assignment_method = default_learner_proficiency_ratingMethod

					llp.learner_language_proficiency_documentation= default_learner_proficiency_ratingMethod
		...
		# Q: is xml or csv more "canonical"?
		text_block_text_file = docid + ".xml"

		# TODO: what is the dakoda id? 
		text_block.text_id = meta.NaString.NOT_AVAILABLE

		# Q: is there a specific text id?
		...
		text_block.text_language = text_lg
		...
		# TODO: check if we do have this info after all
		text_block.text_time_of_creation = meta.NaString.NOT_AVAILABLE

		text_block.text_token_count = -1
		text_block.text_clause_count = -1
		...
		text_learner.text_learner_age_production = mdf["text_learner_ageProduction"].iloc[0]
		# CHECK: what does aggregated mean? does DKD have predefined age-ranges?
		text_learner.text_learner_age_production_aggregated = meta.NaString.NOT_AVAILABLE
		...
		tprof.text_proficiency_official_language_testing_score = meta.NaString.NOT_AVAILABLE
		...
		#Q: The original data has some error anno and corrections but we're not carrying that over.
